...
export async function regenerateDatabase(
  ...
  console.log(`Regenerating database with ${count} documents in collection "${collectionName}"...`);
  ...
  for (let i = 0; i < documents.length; i += batchSize) {
    ...
    if ((i + batchSize) % 10000 === 0 || i + batchSize >= documents.length) {
      console.log(`Inserted ${Math.min(i + batchSize, documents.length)}/${count} documents...`);
    }
  }
  
  console.log(`Successfully regenerated "${collectionName}" with ${count} documents!`);
}
